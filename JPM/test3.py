...
class TranscriptExtractor:
    ...
    def _extract_qa_chunk(self, qa_text: str) -> pd.DataFrame:
        """Extract Q&A data from a single chunk."""
        
        qa_extraction_prompt = """
        You are tasked with extracting Q&A data from an earnings call transcript.
        
        Instructions:
        1. Extract ONLY the Question & Answer section content provided
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-3.5-turbo",  # Use GPT-3.5 for better rate limits
                messages=[
                    {"role": "system", "content": "You are a financial transcript processing expert. Return only valid JSON arrays."},
            ...
            print(f"Error extracting Q&A chunk: {e}")
            return pd.DataFrame()
...
def process_transcript_with_llm(transcript_text: str, openai_api_key: str, output_dir: str = "."):
    """
    Main function to process transcript using LLM prompts with rate limiting.
    ...
    if not qa_dataframe.empty:
        ...
        print("⚠️  No Q&A data extracted - creating empty files")
        empty_df = pd.DataFrame(columns=['question_number', 'type', 'speaker_name', 'speaker_details', 'text'])
        empty_df.to_csv(qa_csv_file, index=False)
        empty_df.to_excel(qa_excel_file, index=False)
    ...
    if not qa_dataframe.empty:
        ...
        print(f"\n⚠️  Q&A extraction failed. Consider using manual prompts instead.")
